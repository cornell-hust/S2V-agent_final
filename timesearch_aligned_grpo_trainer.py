...
def _build_episode_tensor_pack_from_rollout(
    ...
    *,
    ...
    if not isinstance(episode_feature, dict):
        return None
    pack = copy.deepcopy(episode_feature)
    pack["advantages"] = torch.tensor([float(rollout_advantage)], dtype=torch.float32)
    pack["reward_summary"] = copy.deepcopy(rollout.get("reward_summary") or {})
    pack["fecv_failed"] = bool(rollout.get("fecv_failed"))
    pack["fecv_failure_message"] = str(rollout.get("fecv_failure_message") or "")
    return pack
...
class TimesearchAlignedGRPOTrainerMixin:
    def __init__(
        ...
        **trainer_kwargs: Any,
        ...
        self.replay_buffer_enable = False
        self.replay_buffer_type = "none"
        self.replay_buffer_capacity = 0
        self.replay_buffer_alpha = 1.0
    ...
    def _generate_scored_rollouts_batch(
        ...
        *,
        ...
        rollout_chunks = _iter_chunked(list(zip(rollout_meta, rollout_items)), self.rollout_stage_batch_size)
        total_rollout_chunks = len(rollout_chunks)
        for rollout_chunk_index, rollout_chunk in enumerate(rollout_chunks, start=1):
            chunk_meta = [entry[0] for entry in rollout_chunk]
            chunk_items = [entry[1] for entry in rollout_chunk]
            rollout_start_time = time.perf_counter()
            runtime_log(
                f"rl rollout chunk start: chunk={rollout_chunk_index}/{total_rollout_chunks} size={len(chunk_items)}",
                runtime=distributed_runtime_from_env(),
                main_process_only=True,
            )
            generated_rollouts = list(self.rollout_runner.run_episodes(chunk_items, rollout_policy, capture_prompt_messages=True))
            runtime_log(
                f"rl rollout chunk end: chunk={rollout_chunk_index}/{total_rollout_chunks} size={len(chunk_items)} elapsed_sec={time.perf_counter() - rollout_start_time:.3f}",
                runtime=distributed_runtime_from_env(),
                main_process_only=True,
            )
            ...
            fecv_chunks = _iter_chunked(list(zip(fecv_rollout_refs, fecv_batch_inputs)), self.fecv_stage_batch_size)
            total_fecv_chunks = len(fecv_chunks)
            for fecv_chunk_index, fecv_chunk in enumerate(fecv_chunks, start=1):
                chunk_refs = [entry[0] for entry in fecv_chunk]
                chunk_inputs = [entry[1] for entry in fecv_chunk]
                fecv_start_time = time.perf_counter()
                runtime_log(
                    f"rl fecv chunk start: rollout_chunk={rollout_chunk_index}/{total_rollout_chunks} fecv_chunk={fecv_chunk_index}/{total_fecv_chunks} size={len(chunk_inputs)}",
                    runtime=distributed_runtime_from_env(),
                    main_process_only=True,
                )
                try:
                    ...
                    if self.fecv_failure_policy == "fail":
                        raise
                    fecv_results = [exc for _ in chunk_inputs]

                runtime_log(
                    f"rl fecv chunk end: rollout_chunk={rollout_chunk_index}/{total_rollout_chunks} fecv_chunk={fecv_chunk_index}/{total_fecv_chunks} size={len(chunk_inputs)} elapsed_sec={time.perf_counter() - fecv_start_time:.3f}",
                    runtime=distributed_runtime_from_env(),
                    main_process_only=True,
                )
                for (item_index, generation_id, rollout, item), fecv_result in zip(chunk_refs, fecv_results):
                    video_id = str(rollout.get("video_id") or item.get("video_id") or "")
                    try:
    ...
    def _build_generation_item_payload_from_rollouts(
        ...
        for rollout in list(scored_rollouts or []):
            ...
            raw_pack = _build_episode_tensor_pack_from_rollout(
                rollout,
                min_abs_advantage=0.0,
            )
            if raw_pack is not None:
                num_episode_candidates_before_min_weight += 1
            pack = _build_episode_tensor_pack_from_rollout(
                rollout,
            ...
            result = self._build_prepared_batch_from_feature(pack)
            ...
            episode_entries.append(
                {
                    "feature": copy.deepcopy(pack),
                    "episode_input": result.batch,
                }
            )
        episode_inputs: List[Dict[str, Any]] = []
        for entry in episode_entries:
            episode_input = copy.deepcopy(dict(entry["episode_input"] or {}))
            episode_inputs.append(episode_input)
        runtime_stats["raw_local_episode_input_count"] = int(len(episode_inputs))
        runtime_stats["raw_local_prepared_batch_count"] = int(len(episode_inputs))
        runtime_log(
            "rl prepared payload built: "
            f"video_id={video_id} "
            ...
            f"num_batch_build_success={len(episode_inputs)} "
            f"fastpath_old_logprob_batches={len(episode_inputs)} "
    ...
    def _aggregate_generation_step_payload(
        ...
        for payload in item_payloads:
            ...
            episode_inputs.extend(list(payload.get("episode_inputs") or payload.get("prepared_batches") or []))
    ...
    def _compute_liger_loss_for_batch(
        self,
        *,
        model: Any,
        batch: Dict[str, Any],
        ...
        if self.liger_grpo_loss is None:
            return self._compute_sample_losses_for_batch(model=model, batch=batch)
        ...
        outputs = model(
            ...
            **self._prepared_batch_multimodal_inputs(batch),
        )
        hidden_states = getattr(outputs, "hidden_states", None)
        if not hidden_states:
            raise RuntimeError("use_liger_loss=True requires model outputs to expose hidden_states.")
        last_hidden_state = hidden_states[-1][:, -int(completion_ids.shape[1]) :, :]
        ...
        if old_policy_token_log_probs is None:
            ...
            old_policy_token_log_probs = old_policy_token_log_probs.to(last_hidden_state.device, dtype=torch.float32)
        loss, _ = self.liger_grpo_loss(
            _input=last_hidden_state,
            lin_weight=_unwrap_model(model).lm_head.weight,
            selected_token_ids=completion_ids.to(last_hidden_state.device),
            attention_mask=completion_mask.to(last_hidden_state.device),
            advantages=self._prepare_advantages(batch, last_hidden_state.device),
            bias=getattr(_unwrap_model(model).lm_head, "bias", None),
            old_per_token_logps=old_policy_token_log_probs,
            ref_per_token_logps=ref_per_token_logps,
        )
        sample_count = int(completion_ids.shape[0])
        return loss.expand(sample_count)
    ...
    def _prepare_inputs(self, inputs):
        ...
        try:
            ...
            episode_inputs = list(prepared.get("episode_inputs") or prepared.get("prepared_batches") or [])
            try:
                target_device = next(rollout_model.parameters()).device
            except StopIteration:
                target_device = torch.device("cpu")
            episode_inputs = [
                self._move_prepared_batch_to_device(episode_input, device=target_device)
                for episode_input in episode_inputs
            ]
            episode_inputs = self._align_prepared_batches_across_ranks(
                episode_inputs,
                device=target_device,
                runtime_stats=runtime_stats,
            )
            materialize_start = time.perf_counter()
            episode_inputs = self._materialize_prepared_batches(episode_inputs, device=target_device)
            runtime_stats["raw_local_episode_input_count"] = int(len(episode_inputs))
            runtime_stats["raw_local_prepared_batch_count"] = int(len(episode_inputs))
            runtime_log(
                f"rl materialize end: episode_inputs={len(episode_inputs)} elapsed_sec={time.perf_counter() - materialize_start:.3f}",
                runtime=distributed_runtime_from_env(),
                main_process_only=True,
            )
            return {
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        episode_inputs = list(inputs.get("episode_inputs") or inputs.get("prepared_batches") or [])
        ...
        for batch in episode_inputs:
            for microbatch in self._iter_loss_microbatches(batch):
                sample_losses = (
                    self._compute_liger_loss_for_batch(model=model, batch=microbatch)
                    if self.use_liger_loss
                    else self._compute_sample_losses_for_batch(model=model, batch=microbatch)
                )
...
def create_timesearch_aligned_grpo_trainer(
    *,
    ...
    replay_buffer_enable: bool = False,
    replay_buffer_type: str = "none",
