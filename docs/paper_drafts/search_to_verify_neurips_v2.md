...
Most video anomaly understanding (VAU) systems reason over fixed observations — sampled frames, preselected clips, or pre-segmented events — and decode a judgment from that bundle. This fails when anomalies are defined not by a single salient frame but by the completeness of an event chain linking **precursor** cues to a **trigger** and then to **confirmation or aftermath**. We present **Search-to-Verify Agent (S2V-Agent)**, the first trainable pipeline that unifies structured tool use, active event-chain search, policy-internal counterfactual verification, and structured case finalization for VAU. The policy interleaves four executable actions — `scan_timeline`, `seek_evidence`, `verify_hypothesis`, and `finalize_case` — to actively recover missing evidence and test whether the current evidence subset is sufficient and necessary before committing to a final report. Our contribution is a conceptual shift: from fixed-observation reasoning to **event-chain-oriented active inference**, where evidence faithfulness is a first-class optimization objective, not a post-hoc diagnostic. We make three claims: (1) VAU should be reframed as agentic event-chain search; (2) policy-internal counterfactual verification is the right mechanism for readiness gating; and (3) FECV-grounded learning turns evidence faithfulness into a trainable target. We evaluate not only final anomaly prediction but also temporal grounding, event-chain recovery, verification quality, and verify-to-finalize behavior.
...
This limitation becomes structural once anomaly understanding is viewed through the lens of **event-chain completeness**. Many anomalies are not best characterized by one peak frame or even one short event clip. They are better understood as short temporal processes whose meaning depends on whether the system can recover a coherent chain from **precursor** cues to a **trigger**, and then to **confirmation or aftermath**. This is precisely where the current "multi-granularity" framing still falls short: more temporal scales do not by themselves guarantee that the model actively searches for the missing stages of an anomaly case.

This paper takes the next step and formulates VAU as an explicit **search-to-verify** decision process. We introduce **Search-to-Verify Agent (S2V-Agent)**, a constrained tool-using policy that alternates among four executable actions: `scan_timeline`, `seek_evidence`, `verify_hypothesis`, and `finalize_case`. Search is part of the policy rather than an offline preprocessing assumption. Verification is a policy action rather than an external afterthought. Finalization is a structured case report rather than a loose free-form answer.
...
- **S** is the joint state space of dialogue history and evidence ledger: s_t = (history_t, E_t, M_t), with E_t the evidence ledger and M_t the coarse temporal map.
...
R(τ) = α · R_acc(τ) + β · R_fecv(τ) + δ · R_protocol(τ)

where R_acc is answer correctness (category, existence, temporal grounding), R_fecv is evidence faithfulness measured by the counterfactual verification score, and R_protocol is structured finalization compliance (protocol-consistent use of `verify_hypothesis` before `finalize_case`). Default weights are **α = 0.4, β = 0.4, δ = 0.2**, reflecting equal priority between correctness and evidence faithfulness, with a lighter regularization term for protocol compliance.

**Event-chain completeness.** For a recovered chain C, the stage coverage metric is:

stage_coverage(C) = |{s ∈ {pre, trg, conf} : C_s ≠ ∅ ∧ temporally_valid(C_s)}| / 3

where temporally_valid(C_s) requires that the evidence moments in stage s are temporally ordered and consistent with the anomaly interval. A coverage of 1.0 indicates all three stages are populated with temporally valid evidence.
...
The counterfactual verification is **computational**, not merely prompted self-reflection. For each evidence item e_i in the selected set E, the protocol proceeds as follows:

1. **Construct** the reduced set E \ {e_i} by removing e_i from the evidence ledger.
2. **Re-query** the policy with the reduced evidence set, using the same claim and dialogue context.
3. **Compare** the finalization verdict under E versus E \ {e_i}.

If the verdict changes from "sufficient" to any other state → e_i is **necessary**.
If the verdict is unchanged → e_i is **redundant**.

This process is run as an **internal policy call**, not an external judge. The policy itself acts as both the verifier and the counterfactual oracle, which is feasible because the verdict depends only on the evidence ledger state and the current claim — both of which are part of the policy's context window.

**Verdict taxonomy:**
- **sufficient**: all stage-critical evidence is present and temporally valid; the case is ready for finalization.
- **insufficient**: one or more required stage evidence items are missing; the policy should continue searching.
- **misaligned**: retrieved evidence contradicts the current claim; the policy should revise the hypothesis.
- **redundant**: one or more evidence items can be removed without changing the verdict; the policy should refine the evidence set.

```
Algorithm 1: S2V-Agent Inference Episode
Input: Video V, query q, turn budget T_max
Initialize: evidence ledger E ← ∅, temporal map M ← ∅, turn t ← 0
while t < T_max do:
    action ← π(s_t | history, E, M)  // policy selects action
    if action = scan_timeline:
        ...
        verdict ← CounterfactualVerify(claim, E)  // see Protocol above
        if verdict = "sufficient": recommend finalize
        elif verdict = "insufficient": recommend continue_search
        elif verdict = "misaligned": recommend revise_claim
        elif verdict = "redundant": recommend refine_evidence
