...
**Answer correctness reward.** R_acc averages per-field scores across three question families: (i) *multiple-choice* — binary match for existence, category, severity, and counterfactual type; (ii) *temporal grounding* — interval IoU between predicted and target anomaly/precursor intervals; (iii) *open-ended* — LLM-judge-scored textual quality for trigger evidence descriptions, event-chain stage summaries, and anomaly rationale. The final R_acc is the micro-average over all scored fields. The per-field QA Accuracy evaluation metric (Section 5.4) mirrors R_acc's structure but is computed against held-out test targets rather than training rewards, measuring generalization of decision quality.
...
We organize evaluation into **6 primary metrics** — 3 standard (enabling comparison with prior work) and 3 novel (enabled by S2V-Bench) — plus secondary diagnostics in supplementary.
...
Table 1: Main results on S2V-Bench (2,960 videos, 114 categories). We report 6 primary metrics across 3 paradigm groups.
...
| **Search-to-Verify (ours)** | **[TBD]** | **[TBD]** | **[TBD]** | **[TBD]** | **[TBD]** | **[TBD]** |
