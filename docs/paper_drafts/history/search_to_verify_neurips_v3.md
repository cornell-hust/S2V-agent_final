...
Most video anomaly understanding (VAU) systems reason over fixed observations — sampled frames, preselected clips, or pre-segmented events — and decode a judgment from that bundle. This fails when anomalies are defined not by a single salient frame but by the completeness of an event chain linking **precursor** cues to a **trigger** and then to **confirmation or aftermath**. We present **Search-to-Verify Agent (S2V-Agent)**, a trainable pipeline that explicitly unifies structured tool use, active event-chain search, policy-internal counterfactual verification, and structured case finalization for VAU. The policy interleaves four executable actions — `scan_timeline`, `seek_evidence`, `verify_hypothesis`, and `finalize_case` — to actively recover missing evidence and test whether the current evidence subset is sufficient and necessary before committing to a final report. Our contribution is a conceptual shift: from fixed-observation reasoning to **event-chain-oriented active inference**, where evidence faithfulness is a first-class optimization objective, not a post-hoc diagnostic. We make three claims: (1) VAU should be reframed as agentic event-chain search; (2) policy-internal counterfactual verification is a natural mechanism for readiness gating; and (3) FECV-grounded learning turns evidence faithfulness into a trainable target. We evaluate not only final anomaly prediction but also temporal grounding, event-chain recovery, verification quality, and verify-to-finalize behavior.
...
This paper formulates VAU as an explicit **search-to-verify** decision process. We introduce **Search-to-Verify Agent (S2V-Agent)**, a constrained tool-using policy that alternates among four executable actions: `scan_timeline`, `seek_evidence`, `verify_hypothesis`, and `finalize_case`. Search is part of the policy rather than an offline preprocessing assumption. Verification is a policy action rather than an external afterthought. Finalization is a structured case report rather than a loose free-form answer.

Our core novelty is not any single ingredient — active search, verification, or RL training — but their unification into a single trainable pipeline where evidence faithfulness is a first-class optimization objective, not a post-hoc diagnostic. Prior work in VAU has not yet unified structured tool use, active event-chain search, policy-internal counterfactual verification, and structured case finalization into one trainable pipeline.

Our paper makes three claims. First, VAU should be reframed as **agentic event-chain search** rather than fixed-observation reasoning. Second, **policy-internal counterfactual evidence verification** is a natural mechanism for deciding when a case is actually ready to finalize. Third, **FECV-grounded learning** turns evidence faithfulness into a first-class optimization target rather than a post-hoc diagnostic. These claims define a sharper paper story for a NeurIPS-style main paper: the field should move from analyzing event frames to recovering complete event chains, and from passively explaining observations to actively searching and verifying them.
...
with default weights **w_acc = 1.0, w_fecv = 0.35, w_prot = 0.1**. Each component is defined concretely:
...
Evidence that does not satisfy this condition is classified as redundant and should trigger a `refine_evidence` recommendation.
...
S2V-Agent is a constrained tool-using policy for video anomaly understanding, building on the tool-use paradigm established by ReAct [11]. At each turn, the policy reasons over the dialogue state, the current evidence ledger, and previously observed temporal context, and then chooses one of four executable actions: `scan_timeline`, `seek_evidence`, `verify_hypothesis`, or `finalize_case`. This action design is the method's central abstraction. It forces the policy to separate broad temporal coverage from evidential commitment, to expose when it believes the case is or is not ready, and to expose whether the case appears ready before producing a structured anomaly report.
...
- **redundant**: Evidence removal does not degrade the decision, indicating over-collection. Reward: +0.35.

The recommended next action maps directly to the four-action space: sufficient → `finalize_case`; insufficient → `scan_timeline` or `seek_evidence`; misaligned → update the working hypothesis c_t then `seek_evidence`; redundant → `seek_evidence` with tighter stage targeting.
...
```
Algorithm 1: S2V-Agent Inference Episode
...
**Implementation details.** Our policy is instantiated on Qwen2.5-VL-7B as the base multimodal model, fine-tuned through the SFT and RL stages described above. The teacher judge uses Qwen2.5-VL-72B.
