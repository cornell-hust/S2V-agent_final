...
| Paradigm | Representative works | Primary unit of reasoning | Tool-use search policy | Counterfactual verification | Event-chain completeness target | Verify-before-finalize gate |
| --- | --- | --- | --- | --- | --- | --- |
| Fixed-observation VAU | CUVA, AnomalyRuler, HAWK, Holmes-VAU, VERA | sampled frames, clips, or pre-built segments | Not explicit | Not explicit | Partial | Typically absent |
| Reasoning / reflection VAU | VAU-R1, SRVAU-R1, PrismVAU | prepared observation plus stronger reasoning | Not explicit | Implicit | Not central | Typically absent |
| Adjacent agentic VAD | PANDA, QVAD | anomaly search or question-driven inspection | Partial | Implicit | Not the main objective | Partial |
| **Search-to-Verify** | **ours** | **recovered event chain over precursor -> trigger -> confirmation/aftermath** | **Yes** | **Yes** | **Yes** | **Yes** |

*Table entries reflect our reading of the cited papers' primary contributions. We acknowledge that some systems may implicitly perform functions listed as absent; the comparison targets explicit, central design choices rather than emergent behaviors.*
