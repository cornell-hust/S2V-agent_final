# Search-to-Verify: Agentic Video Anomaly Understanding with Counterfactual Evidence Reinforcement Learning
...
Our core novelty is not a fundamentally new learning algorithm, but a **task reframing and structured protocol** for VAU: we reformulate anomaly understanding as event-chain search with explicit verification gating, and we provide a concrete training objective (FECV-grounded reward) that makes evidence faithfulness optimizable rather than merely diagnosable. Prior work in VAU has not yet unified structured tool use, active event-chain search, counterfactual evidence verification, and structured case finalization into one trainable pipeline.

Our paper makes three claims. First, VAU should be reframed as **agentic event-chain search** rather than fixed-observation reasoning. Second, **counterfactual self-consistency checking** — where the policy re-evaluates its claim under evidence perturbations — is a practical readiness proxy for deciding when a case is ready to finalize, while oracle-grounded counterfactual verification provides the training signal. Third, **FECV-grounded learning** turns evidence faithfulness into a first-class optimization target rather than a post-hoc diagnostic. These claims define a sharper paper story for a NeurIPS-style main paper: the field should move from analyzing event frames to recovering complete event chains, and from passively explaining observations to actively searching and verifying them.
