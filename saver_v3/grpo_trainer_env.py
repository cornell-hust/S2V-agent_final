...
class _NativeGRPOTrainerMixin:
        ...
        *,
        ...
        for source_entry in source_entries:
            ...
            if sample_losses is None or sample_losses.numel() <= 0:
                continue
            recovered_trainable_samples += int(
                (
                    self._sample_loss_multiplier(
                        prepared_single,
        ...
        if recovered_losses:
            ...
            return torch.cat(recovered_losses, dim=0), int(recovered_trainable_samples)
        runtime_log(
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        for prepared_batch_index, batch in enumerate(prepared_microbatches, start=1):
            runtime_log(
                "rl debug prepared batch start: "
                f"compute_loss_call={compute_loss_call_index} "
                f"prepared_batch_index={int(prepared_batch_index)}/{int(local_prepared_batch_count)} "
                f"completion_ids_shape={tuple(int(dim) for dim in batch['completion_ids'].shape)} "
                f"merge_signature={repr(self._prepared_batch_merge_signature(batch))}",
                runtime=distributed_runtime_from_env(),
                main_process_only=False,
            )
            ...
            try:
                ...
                if len(source_entries) <= 1:
                    ...
                    sample_losses, recovered_trainable_samples = self._recover_sample_losses_from_source_entries(
                        model=model,
                        batch=batch,
                        batch_error=exc,
                    )
            ...
            total_samples += (
                ...
                if recovered_preweighted
                else int((sample_loss_multiplier > 0).sum().item())
            )
            ...
            runtime_log(
                ...
                f"compute_loss_call={compute_loss_call_index} "
                f"prepared_batch_index={int(prepared_batch_index)}/{int(local_prepared_batch_count)} "
                f"trainable_samples={int((sample_loss_multiplier > 0).sum().item())} "
        ...
        runtime_log(
            ...
            f"compute_loss_call={compute_loss_call_index} "
            f"local_prepared_batch_count={int(local_prepared_batch_count)} "
            f"global_total_samples={int(global_total_samples)}",
            runtime=distributed_runtime_from_env(),
            main_process_only=False,
        )
        final_loss = total_loss_sum * float(world_size) / float(max(1, int(global_total_samples)))
        self._assert_finite_tensor(
            stage="compute_loss_return",
            tensor_name="final_loss",
            tensor_value=final_loss,
            extra={
                "global_total_samples": int(global_total_samples),
