...
def _prepare_messages(
    ...
    *,
    ...
    before_budget = summarize_visual_budget(messages)
    ...
    after_budget = summarize_visual_budget(prepared)
    if (
        any(
            int(value) > 0
            for value in (
                ...
                max_total_images,
                max_tool_message_frames,
                max_total_video_frames,
            )
        )
        and before_budget != after_budget
    ):
        runtime_log(
            "train visual budget debug: "
            ...
            f"max_tool_message_frames={int(max_tool_message_frames) or 'all'} "
            f"max_total_video_frames={int(max_total_video_frames) or 'all'}",
...
def _build_pure_grpo_tensor_pack_from_episode_feature(
    processor: Any,
    feature: Dict[str, Any],
    *,
    max_image_side: int = 0,
...
def _resolve_training_proposal_device(explicit_device: str | None, *, runtime: Any) -> str:
    ...
    if explicit and max(1, int(getattr(runtime, "world_size", 1) or 1)) > 1 and re.fullmatch(r"(?:cuda:\d+|\d+)", explicit.lower()):
        runtime_log(
            (
                "distributed proposal device warning: "
