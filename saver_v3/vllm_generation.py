...
def _extract_vllm_estimated_max_model_len(exc: BaseException) -> int:
    match = _VLLM_ESTIMATED_MAX_MODEL_LEN_RE.search(str(exc or ""))
    if not match:
        return 0
    try:
...
def _construct_vllm_llm_with_max_model_len_retry(
    *,
    llm_ctor: Any,
    ...
    requested_max_model_len = int(llm_kwargs.get("max_model_len") or 0)
    try:
        return llm_ctor(**llm_kwargs)
    except ValueError as exc:
        fallback_max_model_len = _extract_vllm_estimated_max_model_len(exc)
        if fallback_max_model_len <= 0 or fallback_max_model_len >= requested_max_model_len:
            raise
        retry_kwargs = dict(llm_kwargs)
        retry_kwargs["max_model_len"] = int(fallback_max_model_len)
        runtime_log(
            (
                f"{context_label}: retrying vLLM init with reduced max_model_len "
                f"{fallback_max_model_len} (requested {requested_max_model_len}) after KV cache sizing failure: {exc}"
            ),
            runtime=runtime,
            main_process_only=True,
        )
        return llm_ctor(**retry_kwargs)
    ...
    def _build_llm(self) -> Any:
        ...
        try:
            ...
            with _temporary_single_rank_external_launcher_env(self.runtime, tp_size=tp_size):
                return _construct_vllm_llm_with_max_model_len_retry(
                    llm_ctor=LLM,
                    llm_kwargs=llm_kwargs,
                    runtime=self.runtime,
                    context_label="colocated_vllm_runtime",
                )
        finally:
            if disable_v1_multiprocessing:
    ...
    def _build_llm(self) -> Any:
        ...
        try:
            ...
            return _construct_vllm_llm_with_max_model_len_retry(
                ...
                llm_kwargs=llm_kwargs,
                runtime=self.runtime,
                context_label="external_launcher_vllm_runtime",
            )
        finally:
            if previous_v1_multiprocessing is None:
                os.environ.pop("VLLM_ENABLE_V1_MULTIPROCESSING", None)
...
class VllmQwenGenerationPolicy(QwenGenerationPolicy):
    ...
    def generate_from_messages_batch(
        ...
        output_texts = [self.postprocess_generated_output(output_text) for output_text in raw_output_texts]
        if self.capture_rl_token_traces:
            eos_token_id = int(
