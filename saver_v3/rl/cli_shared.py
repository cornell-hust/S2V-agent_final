...
def build_active_rl_arg_parser(*, description: str) -> argparse.ArgumentParser:
    ...
    parser.add_argument("--rl-compute-loss-microbatch-size", type=int, default=3, help="Completion-only forward microbatch size in active GRPO compute_loss().")
    ...
    parser.add_argument("--keep-recent-tool-image-messages", type=int, default=3, help="If >0, keep images for at most N recent tool image messages during RL; when present, the latest scan_timeline image message reserves one slot inside this budget.")
